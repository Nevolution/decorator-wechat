...
public class WeChatDecorator extends NevoDecoratorService {
	...
	private static final String SENDER_PLACEHOLDER = " ";		// Cannot be empty string, or it will be treated as null.
	...
	private Message buildMessage(final CharSequence title, final CharSequence text, final @Nullable CharSequence sender, final boolean group_chat) {
		final int pos_colon;
		...
		return new Message(display_text, 0/* TODO: Any effect in Android Auto? */, group_chat ? display_sender : SENDER_PLACEHOLDER);
	}
	...
	private @Nullable MessagingStyle buildFromArchive(final MutableStatusBarNotification evolving, final Notification n, final CharSequence title,
		...
		for (int i = 0; i < lines.size(); i++) {    // All lines have colon in text
			...
			if (sender_inline && (pos_colon = line.toString().indexOf(SENDER_MESSAGE_SEPARATOR)) > 0)
				messaging.addMessage(line.subSequence(pos_colon + 2, line.length()), when, group_chat ? line.subSequence(0, pos_colon) : SENDER_PLACEHOLDER);
			else messaging.addMessage(line, when, group_chat ? title : SENDER_PLACEHOLDER);
		}
		return messaging;
	}
